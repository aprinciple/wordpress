...
<main class="p-subpage__main">
  ...
  <article class="p-post__intro">
    <div class="container p-post__intro-wrapper">
      <div class="p-post__intro-content">
        <h1 class="p-post__intro-title"><?php the_title(); ?></h1>
        <?php the_field('promo_txt'); ?>
        <div class="p-post__intro-table">
        <?php 
          $table = get_field( 'table_promo' );
          if ( !empty($table) ) {
            echo '<div class="table table-post">';
              if ( !empty( $table['caption'] ) ) {
                echo '<span class="table__caption table-post__caption">' . $table['caption'] . '</span>';
              }
              echo '<div class="table__wrapper table-post__wrapper">';
                if ( !empty( $table['header'] ) ) {
                  echo '<div class="table__row table__head">';
                    foreach ( $table['header'] as $th ) {
                      echo '<span class="table__header table-post__header">';
                        echo $th['c'];
                      echo '</span>';
                    }
                  echo '</div>';
                ...
                foreach ( $table['body'] as $tr ) {
                  echo '<div class="table__row">';
                    foreach ( $tr as $td ) {
                      echo '<span class="table__cell table-post__cell">';
                        echo $td['c'];
                      echo '</span>';
                    }
                  echo '</div>';
  ...
  <div 
    class="p-post__content"
    style="<?php 
      if( get_field('is_content') ) {
        echo 'display:none;';
      } else {
        echo 'display:block;';
      }
